...
# ╔═╡ 9fbb4515-647c-4402-bd2c-8faad98a1e84
function secant(f, x, tol, max_iter=1000)
	x₀, x₁ = x
    iter = 0
    for i ∈ 1:max_iter
        iter += 1
...
begin
	resβ = bisection(f, (a, b), tol)
	resν = newton_raphson(f, f′, x₀, tol)
	resσ = secant(f, (x₀, x₁), tol)

	md"""
	The _Bisection_ method found root **$(resβ[1])** after **$(resβ[2])** iterations. \
	The _Newton-Raphson_ method found root **$(resν[1])** after **$(resν[2])** iterations. \
	The _Secant_ method found root **$(resσ[1])** after **$(resσ[2])** iterations.
	"""
end
...
begin
	...
	\
	The _Bisection_ method found root **$(approx(resβ[1]))** after **$(resβ[2])** iterations. \
	The _Newton-Raphson_ method found root **$(approx(resν[1]))** after **$(resν[2])** iterations. \
	The _Secant_ method found root **$(approx(resσ[1]))** after **$(resσ[2])** iterations.
